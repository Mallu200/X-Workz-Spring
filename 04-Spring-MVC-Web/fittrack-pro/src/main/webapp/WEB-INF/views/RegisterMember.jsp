...
<head>
    <meta charset="UTF-8">
    <title>FitTrack Pro | ${dto.id == 0 ? 'Register' : 'Edit'}</title>
    <style>
        /* Existing styles maintained for professional look */
        ...
        .alert-error {
            background-color: #ffdce0; color: #e74c3c; padding: 12px;
            border-radius: 6px; border: 1px solid #fac5cd; margin-bottom: 20px;
...
<div class="form-container">
    <h2>
        <%-- Matches Controller logic: id=0 is new registration --%>
        <c:choose>
            <c:when test="${dto.id == 0}">New Member Registration</c:when>
            <c:otherwise>Update Member Profile</c:otherwise>
        </c:choose>
    </h2>
    ...
    <form:form action="${pageContext.request.contextPath}/member/save" method="POST" modelAttribute="dto">
        ...
        <%-- Crucial: Path must match the 'id' field in MemberDTO --%>
        <form:hidden path="id" />
        ...
        <div class="form-group">
            ...
            <form:input path="phoneNumber" placeholder="10-digit number" />
        ...
        <div class="form-group">
            ...
            <form:select path="planType">
                <form:option value="" label="-- Choose a Plan --" />
        ...
        <button type="submit" class="btn-submit">
            ${dto.id == 0 ? 'Register Now' : 'Save Changes'}
        </button>
    </form:form>
